...
namespace AjaxCleaningHCM.Core.MasterData.Service
{
    ...
    {
        private readonly IRepositoryBase<Employee> _productRepository;
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IMapper _mapper;
        public EmployeeService(IRepositoryBase<Employee> productRepository, IHttpContextAccessor httpContextAccessor, IMapper mapper)
        {
            _httpContextAccessor = httpContextAccessor;
            _productRepository = productRepository;
            _mapper = mapper;
        }
        public async Task<EmployeeResponseDto> CreateAsync(EmployeeDto request)
        {
            try
            {
                var product = _mapper.Map<Employee>(request);
        ...
        public async Task<OperationStatusResponse> DeleteAsync(long id)
        {
            try
            {
                ...
                var productDto = _mapper.Map<Employee>(product);
        ...
        public async Task<EmployeesResponseDto> GetAllAsync()
        {
            try
            {
                var result = new EmployeesResponseDto();
                var productResponses = await _productRepository.WhereAsync(x => x.RecordStatus == RecordStatus.Active);
                var productDTOs = _mapper.Map<List<EmployeeDto>>(productResponses);
                result.Status = OperationStatus.SUCCESS;
                result.Message = "Operation Successfully Completed";
                result.EmployeeDtos = productDTOs;
                return result;
            }
            catch (Exception ex)
            {
                return new EmployeesResponseDto { Message = "Error Has Occurred While Processing Your Request", Status = OperationStatus.ERROR };
            }
        }
        public async Task<EmployeeResponseDto> GetByIdAsync(long id)
        {
            try
            {
                ...
                var productDTO = _mapper.Map<EmployeeDto>(product.FirstOrDefault());
                result.EmployeeDto = productDTO;
                result.Status = OperationStatus.SUCCESS;
                result.Message = "Operation Successfully Completed";
                return result;
            }
        ...
        public async Task<EmployeeResponseDto> UpdateAsync(EmployeeDto request)
        {
