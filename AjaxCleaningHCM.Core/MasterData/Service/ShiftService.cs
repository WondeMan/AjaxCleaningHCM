...
namespace AjaxCleaningHCM.Core.MasterData.Service
{
    ...
    {
        private readonly IRepositoryBase<Shift> _productRepository;
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IMapper _mapper;
        public ShiftService(IRepositoryBase<Shift> productRepository, IHttpContextAccessor httpContextAccessor, IMapper mapper)
        {
            _httpContextAccessor = httpContextAccessor;
            _productRepository = productRepository;
            _mapper = mapper;
        }
        public async Task<ShiftResponseDto> CreateAsync(ShiftDto request)
        {
            try
            {
                var product = _mapper.Map<Shift>(request);
                ...
                {
                    return new ShiftResponseDto { Message = "Operation Successfully Completed", Status = OperationStatus.SUCCESS };
                }
                else
                {
                    return new ShiftResponseDto { Message = "Error Has Occurred While Processing Your Request", Status = OperationStatus.ERROR };
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<OperationStatusResponse> DeleteAsync(long id)
        {
            try
            {
                ...
                var productDto = _mapper.Map<Shift>(product);
        ...
        public async Task<ShiftsResponseDto> GetAllAsync()
        {
            try
            {
                var result = new ShiftsResponseDto();
                var productResponses = await _productRepository.WhereAsync(x => x.RecordStatus == RecordStatus.Active);
                var productDTOs = _mapper.Map<List<ShiftDto>>(productResponses);
                result.Status = OperationStatus.SUCCESS;
                result.Message = "Operation Successfully Completed";
                result.ShiftDtos = productDTOs;
                return result;
            }
            catch (Exception ex)
            {
                return new ShiftsResponseDto { Message = "Error Has Occurred While Processing Your Request", Status = OperationStatus.ERROR };
            }
        }
        public async Task<ShiftResponseDto> GetByIdAsync(long id)
        {
            try
            {
                var result = new ShiftResponseDto();
                var product = await _productRepository.WhereAsync(x => x.Id == id && x.RecordStatus == RecordStatus.Active);
                if (product == null)
                    return new ShiftResponseDto { Status = OperationStatus.ERROR, Message = "Record Does Not Exist" };
                var productDTO = _mapper.Map<ShiftDto>(product.FirstOrDefault());
                result.ShiftDto = productDTO;
                result.Status = OperationStatus.SUCCESS;
                result.Message = "Operation Successfully Completed";
                return result;
            }
        ...
        public async Task<ShiftResponseDto> UpdateAsync(ShiftDto request)
        {
